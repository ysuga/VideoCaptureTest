================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: VideoCaptureTest プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した VideoCaptureTest には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには VideoCaptureTest アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

VideoCaptureTest.vcproj
...
VideoCaptureTest.h
   ...
   CVideoCaptureTestApp アプリケーション クラスを宣言します。

VideoCaptureTest.cpp
   これは、アプリケーション クラス CVideoCaptureTestApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\VideoCaptureTest.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル VideoCaptureTest.rc に含まれていま
   す。

res\VideoCaptureTest.rc2
...
VideoCaptureTestDlg.h, VideoCaptureTestDlg.cpp - ダイアログ
   これらのファイルは CVideoCaptureTestDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な VideoCaptureTest.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   VideoCaptureTest.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
